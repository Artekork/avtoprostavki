...
// showProducts([
    ...
    function showProducts(productsToShow) {
        let productContainer = document.querySelector('.product_list');
        productContainer.innerHTML = ''; // Очищаем контейнер перед отображением новых продуктов
        let showMoreButton = document.querySelector(".switch-pages__btn");
        let startIndex = 0; // Начальный индекс для отображения продуктов
        let productsToDisplay = productsToShow.slice(startIndex, startIndex + countToShow); // Выбираем первые countToShow продуктов для отображения

        productsToDisplay.forEach(productId => {
            // Логика по созданию карточки продукта остается той же
            createProductCard(productId, productContainer);
        });

        // Функция обработчика события
        showMoreHandler = function() {
            startIndex += countToShow; // Увеличиваем начальный индекс
            let nextProductsToDisplay = productsToShow.slice(startIndex, startIndex + countToShow);
            nextProductsToDisplay.forEach(productId => {
                createProductCard(productId, productContainer);
            });

            // Если больше нет продуктов для отображения, скрываем кнопку "Показать ещё"
            if (startIndex + countToShow >= productsToShow.length) {
        ...
        // Обновляем состояние кнопки в зависимости от количества продуктов
        function updateShowMoreButton() {
            // Если есть еще продукты в списке, показываем кнопку "Показать ещё", иначе скрываем
            if (startIndex + countToShow < productsToShow.length) {
        ...
        // Добавляем обработчик события один раз при инициализации
    ...
    function createProductCard(productId, productContainer) {
        let prostavkiRef = database.ref('prostavki/' + productId);
        prostavkiRef.once('value')
            .then(function(snapshot) {
                var prostavkaData = snapshot.val();
    ...
    function populateCarModels(brand) {
        ...
        get(modelsRef).then((snapshot) => {
            if (snapshot.exists()) {
                ...
                // Add default option
                const defaultOption = document.createElement('option');
                defaultOption.text = 'Выберите модель';
    ...
    function populateCarYears(brand, model) {
        ...
        get(yearsRef).then((snapshot) => {
            if (snapshot.exists()) {
                ...
                // Add default option
                const defaultOption = document.createElement('option');
                defaultOption.text = 'Выберите год';
...
//   })
  ...
  this.document.querySelector('.product-list__btn-filter').addEventListener('click', function(){
    let filter_menu = document.querySelector('.filter__fields')
    let overlay = document.querySelector('.overlay')
    let button_filter = document.querySelector('.product-list__btn')
    
    overlay.style.display = 'block'
    
    filter_menu.style.transform = 'translateX(0%)'

    function closeOverlay() {
        overlay.style.display = 'none'
        filter_menu.style.transform = 'translateX(-100%)'
        overlay.removeEventListener('click', closeOverlay)
        button_filter.removeEventListener('click', closeOverlay)
    }

    overlay.addEventListener('click', closeOverlay)
    button_filter.addEventListener('click', closeOverlay)
  })

  this.window.addEventListener('resize', function() {
    if (window.innerWidth > 440) {
        document.querySelector('.filter__fields').style.transform = 'translateX(0%)';
    } else {
        document.querySelector('.filter__fields').style.transform = 'translateX(-100%)';
